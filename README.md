...
[![NPM version](https://img.shields.io/npm/v/@snag/sdk.svg)](https://npmjs.org/package/@snag/sdk) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/@snag/sdk)

This library provides convenient access to the Snag Solutions REST API from server-side TypeScript or JavaScript.

The REST API documentation can be found on [docs.snag-solutions.com](https://docs.snag-solutions.com). The full API of this library can be found in [api.md](api.md).
...
async function main() {
  ...
  const asset: SnagSolutions.AssetCreateResponse = await client.assets.create(params);
}
...
```ts
async function main() {
  const asset = await client.assets.create({ fileName: 'REPLACE_ME' }).catch(async (err) => {
...
await client.assets.create({ fileName: 'REPLACE_ME' }, {
  maxRetries: 5,
});
```
...
const response = await client.assets.create({ fileName: 'REPLACE_ME' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: asset, response: raw } = await client.assets.create({ fileName: 'REPLACE_ME' }).withResponse();
...
To do the inverse, add `import "@snag/sdk/shims/node"` (which does import polyfills).
