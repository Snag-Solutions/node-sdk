...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['SNAG_SOLUTIONS_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new SnagSolutions({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['SNAG_SOLUTIONS_BASE_URL'] = 'https://example.com/from_env';
    ...
    test('in request options overridden by client options', () => {
      const client = new SnagSolutions({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['SNAG_SOLUTIONS_BASE_URL'] = 'http://localhost:5000/env';
      const client = new SnagSolutions({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new SnagSolutions({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new SnagSolutions({ apiKey: 'My API Key' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new SnagSolutions({ apiKey: 'My API Key', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new SnagSolutions({ apiKey: 'My API Key', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new SnagSolutions({ apiKey: 'My API Key', fetch: testFetch });
